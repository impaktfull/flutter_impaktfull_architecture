...
import 'package:impaktfull_architecture/src/model/data/error/network_error.dart';
...
abstract class SimpleInterceptor {
  ...
  ///
  /// Any other values will simply call the next interceptor with [options]
  Future<Object?> onResponse(Response<dynamic> response) {
...
class CombiningSmartInterceptor implements Interceptor {
  ...
  Future<void> onError(
    ...
    BaseNetworkError<dynamic>? finalResult;
    for (final interceptor in _interceptors.reversed) {
      try {
        ...
        if (res is BaseNetworkError) {
          finalResult = res;
        }
      } on BaseNetworkError<dynamic> catch (e) {
        finalResult = e;
